...
export function JudgeFeedback({
  ...
  // Collapse toggle only available when evaluation is complete
  const canCollapse = !!scores && !isStreaming;
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {canCollapse && (
          <button
            ...
            className="text-xs text-purple-400 hover:text-purple-700 transition-colors font-medium"
          >
            {collapsed ? "Show ↓" : "Hide ↑"}
          </button>
        )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(["for", "against"] as const).map((pos) => {
                  const detail =
                    pos === "for" ? scores.detail_for : scores.detail_against;
                  const color =
                    pos === "for" ? "text-blue-700" : "text-red-700";
                  const label = pos === "for" ? "FOR" : "AGAINST";
                  const roundScore =
                    pos === "for" ? scores.score_for : scores.score_against;
                  return (
                    <div
                      key={pos}
