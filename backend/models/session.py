...
class DebateSession(Base):
    ...
    mode: Mapped[str] = mapped_column(
        SAEnum("user_vs_ai", "ai_vs_ai", name="debate_mode"), nullable=False
    )
    topic: Mapped[str] = mapped_column(String, nullable=False)
    user_position: Mapped[str | None] = mapped_column(
        SAEnum("for", "against", name="debate_position"), nullable=True
    )
    total_rounds: Mapped[int] = mapped_column(Integer, default=3)
    current_round: Mapped[int] = mapped_column(Integer, default=0)
    status: Mapped[str] = mapped_column(
        SAEnum("pending", "in_progress", "completed", "abandoned", name="session_status"),
        default="pending",
    )
    winner: Mapped[str | None] = mapped_column(
        SAEnum("for", "against", "tie", name="debate_winner"), nullable=True
    )
